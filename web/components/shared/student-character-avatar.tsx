...
export function StudentCharacterAvatar({ name, className, compact = false }: StudentCharacterAvatarProps) {
  ...
  const bgId = `student-avatar-bg-${id}`
  const shirtId = `student-avatar-shirt-${id}`
  const hairId = `student-avatar-hair-${id}`
  const faceId = `student-avatar-face-${id}`
    ...
    >
      <svg viewBox="0 0 120 120" className="h-full w-full" aria-hidden="true">
        <defs>
          ...
          <radialGradient id={faceId} cx="50%" cy="42%" r="55%">
        ...
        <g transform={compact ? 'translate(0 5)' : 'translate(0 0)'}>
          <path d="M29 57 C29 33 43 17 61 17 C80 17 92 33 92 57 C92 78 79 90 60 90 C42 90 29 78 29 57 Z" fill={`url(#${hairId})`} />
          <path d="M31 57 C34 40 40 28 54 23 C50 36 42 44 31 57 Z" fill="#25140F" opacity="0.85" />
          <path d="M36 51 C36 35 47 25 61 25 C75 25 84 36 84 51 L84 60 C84 76 74 85 60 85 C46 85 36 76 36 60 Z" fill={`url(#${faceId})`} />
          ...
          <path d="M28 104 C34 88 45 82 60 82 C75 82 86 88 92 104 L28 104 Z" fill={`url(#${shirtId})`} />
