...
func main() {
	...
	if migrationsDir == "" {
		candidates := []string{"/app/migrations", "/migrations", "./migrations", "../migrations", "../../migrations"}
	...
	if migrationsDir != "" {
		...
		log.Warn().Msg("no migrations directory found — checked /app/migrations, /migrations, ./migrations, ../migrations, ../../migrations")
	}
